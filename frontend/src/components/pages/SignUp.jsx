...
import { createUser, getUserStatus } from "../slices/user/userSlice";
  ...
  return (
    <div className="container mx-auto flex justify-center p-4 mb-8 sm:p-6">
      <ToastContainer
        ...
        theme="colored"
      />
      <form onSubmit={submitHandle} className="overflow-hidden w-[600px] bg-white rounded-md shadow-xl text-slate-500 shadow-slate-200 mt-8">
        <div className="p-6">
          ...
          <div className="flex flex-col space-y-8">
            <div className="relative mt-6">
              <input
                ...
                className="relative w-full h-10 px-4 text-sm placeholder-transparent transition-all border rounded outline-none peer border-slate-200 text-slate-500 autofill:bg-white focus:border-blue-500 focus:outline-none invalid:focus:border-pink-500 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
              />
              <label
                htmlFor="name"
            ...
            <div className="relative my-6">
              <input
                ...
                className={`relative peer w-full h-10 px-4 text-sm placeholder-transparent transition-all border rounded outline-none peer border-slate-200 autofill:bg-white focus:border-blue-500 focus:outline-none ${validation ? '' : 'focus:border-pink-500'} disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400`}
              />
              <label
                htmlFor="id-b03"
                className={`absolute left-2 -top-2 z-[1] px-2 text-xs text-slate-400 transition-all before:absolute before:top-0 before:left-0 before:z-[-1] before:block before:h-full before:w-full before:bg-white before:transition-all peer-placeholder-shown:top-2.5 peer-placeholder-shown:text-sm peer-required:after:content-['\x00a0*'] peer-focus:-top-2 peer-focus:text-xs ${validation ? 'peer-focus:text-blue-500' : 'peer-focus:text-pink-500'} peer-disabled:cursor-not-allowed peer-disabled:text-slate-400 peer-disabled:before:bg-transparent`}
              >
                Password
              </label>
              <small className={`absolute flex justify-between w-full px-4 py-1 text-xs transition ${validation ? 'text-slate-400' : 'peer-focus:text-pink-500'} `}>
